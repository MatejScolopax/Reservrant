...
public class ReservrantDBHelper extends SQLiteOpenHelper {
    ...
    private static final String SQL_CREATE_TABLE_CUSTOMERS= "CREATE TABLE " +
            DatabaseContract.TABLE_NAME_CUSTOMERS + " (" +
    ...
    private static final String SQL_CREATE_TABLE_TABLES = "CREATE TABLE " +
            DatabaseContract.TABLE_NAME_TABLES + " (" +
            ...
            "FOREIGN KEY ("+DatabaseContract.TableTables.COL_ID_CUSTOMER+") REFERENCES "+ DatabaseContract.TABLE_NAME_CUSTOMERS +"("+DatabaseContract.TableCustomers._ID+") )";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL("DROP TABLE IF EXISTS " + DatabaseContract.TABLE_NAME_CUSTOMERS);
        db.execSQL("DROP TABLE IF EXISTS " + DatabaseContract.TABLE_NAME_TABLES);
        onCreate(db);
    }
    ...
    private void readCustomersFromResources(SQLiteDatabase db) throws IOException, JSONException {
        ...
        InputStream in = resources.openRawResource(R.raw.customer_list);
        ...
        for (int i = 0; i < customers_array.length(); i++) {
            ...
            values.put(DatabaseContract.TableCustomers.COL_ID, item.getLong(Customer.JSON_KEY_ID));
            ...
            db.insert(DatabaseContract.TABLE_NAME_CUSTOMERS, null, values);
        }
    }
    ...
    private void readTablesFromResources(SQLiteDatabase db) throws IOException, JSONException {
        StringBuilder builder = new StringBuilder();
        InputStream in = resources.openRawResource(R.raw.table_map);
        BufferedReader reader = new BufferedReader(new InputStreamReader(in));
        ...
        for (int i = 0; i < tables_array.length(); i++) {
            ...
            db.insert(DatabaseContract.TABLE_NAME_TABLES, null, values);
        }
    }
}
